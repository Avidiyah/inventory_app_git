...
**The response is the record (W4).** A completed week is stored as the JSON
of `HubReportResponse` and served back from the row on every later request.
`frozen_at` is the only field the stored copy does not carry; it is filled
from the row.
