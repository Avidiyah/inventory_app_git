...
function distributionCard(distribution, statuses) {
  ...
  return `<article class="hub-graph-card"><h3>${escapeHtml(distribution.label)}</h3>${donutSvg(distribution, statuses)}<ul class="hub-graph-legend">${rows}</ul></article>`;
}
