...
- [ ] **Step 1:** Add the three factories; re-point `hubCrew()`'s `technicians` default at `hubCrewTechnician()`.
- [ ] **Step 2:** Three drift rows after `["hubStockedRequest", …]`.
- [ ] **Step 3:** `npx vitest run tests/frontend/unit/api.endpoints.test.js` — green, three more tests.
- [ ] **Step 4:** Commit `test(p6b): crew technician, attention and on-clock row factories`.
...
- [ ] **Roll-ups, supervisor:** `led {total: 4, in_progress: 2}`, `crew_on_clock: 2`, `crew_total: 5`, `crew_minutes_today: 185` → three tiles in order: `Work orders I lead` / `4` / `2 in progress`; `Crew on the clock` / `2 of 5` / no sub; `Crew time today` / `3 h 5 m` / `ticking`.
- [ ] **Roll-ups, quiet crew:** `in_progress: 0` → no sub on the first tile; `crew_on_clock: 0` → `0 of 5` and no `ticking`.
- [ ] **Roll-ups, techfm_oa:** `Crew on the clock` dropped — two tiles, `Work orders I lead` then `Crew time today`.
- [ ] **Attention:** empty → no `.hub-attention`; two items → the label contains `Needs attention`, its tip is `hub.attention`, `.hub-tile-count` is `2`, and each row is `⚠` + `subject — detail` (`.hub-attention-row span:last-child`), the subject escaped.
- [ ] **Card, on clock:** `running_session` started 75 min before the payload's `server_now` → `.hub-crew-card-on`, `.hub-crew-dot-on`, `ON CLOCK`, `.hub-crew-subject` `WO 7001`, `.hub-crew-running` `running 1 h 15 m`.
- [ ] **Card, off clock:** no `.hub-crew-card-on`, `OFF CLOCK`, subject `Last worked <weekday h:mm>` computed in the test with the module's own `toLocaleString` options; `last_worked: null` → `Last worked Never`.
- [ ] **Card, numbers:** `minutes_today: 120` → `Today` + `2 h 0 m` in the `<strong>`; `Assigned 3 · In-prog 1 · Ready 2`.
- [ ] **Card, flags** (`it.each`): `["long_session", "approaching_cap", "assigned_idle"]` → `⚠ long session`, `⚠ approaching cap`, `⚠ idle`; `["weird_new_flag"]` → `⚠ weird new flag` (the snake-case fallback); `[]` → no `.hub-crew-flags`.
- [ ] **Card, name:** both names blank → `Unknown`; `<b>Bob</b>` renders literally with no `<b>` node.
- [ ] **No crew:** `technicians: []` → the `My crew` label, the "No one is currently routed to you." hint, no `.hub-crew-grid`.
- [ ] **Through `openHub` (D16, both sides):** supervisor with `led.total: 0` → the board still renders (three tiles); techfm_oa with `led.total: 0` → `el.crewMount().innerHTML === ""`; techfm_oa with `led.total: 1` → the board, two tiles.

- [ ] Run the file; commit `test(p6b): hubSupervisor — roll-ups, attention, the card matrix, the D16 absent board`.
...
- [ ] **On the clock, empty:** `Nobody is on the clock right now.`, `.hub-tile-count` `0`, no `.hub-onclock-list`; the label's tip is `hub.clock-attention`.
- [ ] **On the clock, rows:** `technician_name`, subject `WO 7001 · Scholars`; `community: null` → `WO 7001`; `.hub-onclock-elapsed` starts with `formatHm(elapsed_minutes)` (95 → `1 h 35 m`); `flag: "long_session"` → a `.hub-attention-icon` reading `⚠ long session`; an unknown flag → `⚠ odd flag`; `flag: null` → no icon in that row. **Finding:** `hubAdmin.js`'s `FLAG_LABELS` omits `assigned_idle`, so the same domain flag reads `idle` on the crew board and `assigned idle` here — assert both and file it.
- [ ] **Time tiles:** `Supervisor Time` and `Technician Time` through `formatHm`.
- [ ] **Pipeline, shape:** six tiles in order, `data-status` = `created assigned in_progress ready_to_complete completed review`, each label equal to that value's `<option>` text read out of `backend/static/pages/work-orders.html` with `readFileSync` (the module's own drift claim), values off the payload. **Finding:** the page offers `on_hold`, which the pipeline has no tile for — file it.
- [ ] **Pipeline, flag:** `ready_to_complete: 2` → a `⚠` in that tile's label only; `0` → none anywhere.
- [ ] **Pipeline, click:** a tile click → `#work-orders-status-filter` is that status, `#work-orders-page` is active, and one `GET /work-orders/` carrying `status=review`.
- [ ] **`pipelineBound` once-guard:** `connectHub()`, two `labor.session.changed` emits (each remounts the summary on the same node — `mountHubDashboard` is what replaces it, and nothing here rebuilds the tab), then one tile click → exactly one list request.
- [ ] **Exceptions:** five rows in order — `Inventory recounts`, `Missing price / link`, `Catalogue requests` as `N open`; `Admin review queue`, `Stale > 3 days` bare.
- [ ] **Billing:** `Materials` / `Labor` / `Total` through `formatMoney` (asserted by `/1,234\.50/`-style regex, as `unit/format.test.js` does), `.hub-billing-total` on the Total row; `avg_days_to_complete: null` → `—`, `3.4` → `3.4 d`; the sparkline `[0, 4, 8]` → `▁▄█`, all zeros → `▁▁▁`, `[]` → an empty span; `legacy_live_count: null` → five rows, `7` → a sixth `Legacy work orders live` / `7`.

- [ ] Run the file; commit `test(p6b): hubAdmin — on-the-clock rows, the pipeline hand-off and once-guard, exceptions and billing`.

---

### Task 4: Findings, docs, parent plan

- [ ] `docs/open-work.md` → `N-P6-CHARACTERIZED`: the two-vocabulary flag map; the missing `on_hold` pipeline tile; anything else the run surfaces.
- [ ] `docs/current-state.md`: the Vitest bullet names the two new files; count / time updated.
- [ ] Parent plan: tick the P6b bullets; suite-budget row `P6b | N / 45 | S s`.
- [ ] `npm test` to completion; commit `docs: record P6b — the crew board and the admin summary`.

## Done when

- [ ] Four commits, each green.
- [ ] Both exports called by name through the shell: `mountHubCrew`, `mountHubAdminSummary`.
