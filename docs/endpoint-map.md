...
| H8 | GET | `/hub/attendance/week?week=` | **admin only** | `hub.py` → `work_order_report.resolve_week` + `attendance_week.week_payload` → `labor_day.day_bounds`/`overlap_minutes` | attendance_punches (r), users (r) — side-effect-free: no sweep, no row locks, no commit | `apiGetHubAttendanceWeek` | `hubTimesheetsTab.js`, `hubAttendanceHours.js` |
