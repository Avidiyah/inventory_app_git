...
| Attendance (punches, Home tab, Admin Hours + Charged vs clocked) | `domain/attendance.py`, `services/attendance.py` (punch writes), `services/attendance_week.py` (the clocked week read), `services/attendance_compare.py` (clocked + charged, and the CSV), `routers/attendance.py`, `routers/hub.py` (`GET /hub/attendance/week`, `GET /hub/attendance/export`, + the three punch writes, **admin only**), `schemas/attendance.py`, `static/views/hubHome.js`, `static/views/hubTimesheetsTab.js` (tabpanel shell + sub-nav + one shared week payload + the write calls), `static/views/hubAttendanceHours.js`, `static/views/hubAttendanceCompare.js`, `static/views/hubAttendancePunchEditor.js`, `static/views/workOrderActions.js` (stale-punch self-close on Start), `static/api.js` | `test_attendance_*.py`, `test_attendance_week_service.py`, `test_attendance_week_router.py`, `test_attendance_compare_service.py`, `test_attendance_export_router.py`, `test_attendance_admin_service.py`, `test_attendance_admin_router.py`, `test_route_role_gates.py`, `tests/frontend/views/hubHome.test.js`, `hubAttendanceHours.test.js`, `hubAttendanceCompare.test.js`, `hubAttendancePunchEditor.test.js`, `hubTimesheetsTab.test.js`, `workOrderStartPunch.test.js`. The week read serves both Admin sub-tabs from one payload and is side-effect-free — no sweep, no row locks; a forgotten work-order clock is clipped on read by `work_orders.capped_session_end`. It borrows `work_order_report.resolve_week` + `labor_day.day_bounds`. A cross-midnight punch is owned by the day it started, is marked `carried` on the next, and is editable only on the day it started. An Admin edits, adds, and deletes punches from the Hours drill-down; every write lands one `attendance_punch_edits` row per field in the same transaction, any write clears `needs_review`, and a delete is **soft** (`deleted_at`). The grid refetches the week after a write rather than patching itself |
