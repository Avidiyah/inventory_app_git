...
| Attendance (punches, Home tab, Admin Timesheets: Hours + Charged vs clocked + the live roster) | `domain/attendance.py`, `services/attendance.py` (punch writes), `services/attendance_week.py` (the clocked week read), `services/attendance_compare.py` (clocked + charged, and the CSV), `routers/attendance.py`, `services/attendance_live.py` (the live roster), `routers/attendance.py`, `routers/_attendance_events.py` (`attendance.changed`), `routers/hub.py` (`GET /hub/attendance/week`, `/export`, `/live`, + the three punch writes, **admin only**), `schemas/attendance.py`, `static/views/hubHome.js`, `static/views/hubTimesheetsTab.js` (tabpanel shell + sub-nav + one shared week payload + the write calls), `static/views/hubAttendanceHours.js`, `static/views/hubAttendanceCompare.js`, `static/views/hubAttendanceRoster.js`, `static/views/hubAttendancePunchEditor.js`, `static/views/workOrderActions.js` (stale-punch self-close on Start), `static/api.js` | `test_attendance_*.py`, `test_attendance_week_service.py`, `test_attendance_week_router.py`, `test_attendance_compare_service.py`, `test_attendance_export_router.py`, `test_attendance_live_service.py`, `test_attendance_live_router.py`, `test_attendance_admin_service.py`, `test_attendance_admin_router.py`, `test_route_role_gates.py`, `tests/frontend/views/hubHome.test.js`, `hubAttendanceHours.test.js`, `hubAttendanceCompare.test.js`, `hubAttendanceRoster.test.js`, `hubAttendancePunchEditor.test.js`, `hubTimesheetsTab.test.js`, `workOrderStartPunch.test.js`. The week read serves both Admin sub-tabs from one payload and is side-effect-free — no sweep, no row locks; a forgotten work-order clock is clipped on read by `work_orders.capped_session_end`. It borrows `work_order_report.resolve_week` + `labor_day.day_bounds`. A cross-midnight punch is owned by the day it started, is marked `carried` on the next, and is editable only on the day it started. An Admin edits, adds, and deletes punches from the Hours drill-down; every write lands one `attendance_punch_edits` row per field in the same transaction, any write clears `needs_review`, and a delete is **soft** (`deleted_at`). The grid refetches the week after a write rather than patching itself. The Timesheets tab is **Admin+** with exactly two sub-tabs; `GET /hub/timesheets` and the Supervisor crew grid are retired (D6) — a Supervisor keeps the Dashboard crew board. Above Charged vs clocked sits the live roster from `GET /hub/attendance/live`: green charging / yellow idle / red idle past `IDLE_RED_MINUTES`, sorted red→yellow→green longest-idle-first, with everybody unclocked behind an `N not clocked in` footer. It is side-effect-free too, so the sub-tab polls it on the hub's existing 60 s timer and on `attendance.changed` (audience Admin, emitted by all six punch writes); the strip ticks client-side from `domain.attendance.idle_anchor` plus the server skew, and an open labor session past `LABOR_SESSION_MAX_MINUTES` is not charging |
...
- The Charged vs clocked sub-tab puts clocked beside charged (real wall-clock
  on a work-order clock) and prints their difference as **off job**, floored
  at zero. Charged time no punch covers is flagged with a glyph *and* its
  words, never refused (§9). A hand-entered labor adjustment has no start and
  no stop, so it rides beside both and is counted in neither. There is no
  billed column: `billed_labor_minutes` rounds a *work order's combined*
  labor across people and days, so no honest per-person-per-day billed figure
  exists.
...
- Vitest (`npm test`, repo root): `static/` modules under jsdom, mounted on the
  ...
  | `helpers/hub.js` | `views/userHub.js` + 5 dashboard leaves + 3 lazy tabs | per-role requests, tab switching, per-tab failure isolation, the 60 s safety interval and visibility lifecycle, three realtime subscriptions; then each leaf on the same fixture -- the clock, the timeline's local-time math, the three Priorities shapes, the crew board, the admin summary and its pipeline tiles, the Hours grid and the comparison with its roster and CSV, the graph drills, the report's three sections |
