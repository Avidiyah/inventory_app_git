...
|Mass staging — saved work orders (cards-first gate)|✅ Working — the Scan / Stock gate is cards-first for Supervisor+: tap a saved work order (an active stage's room) to dispense into it, or "Add a new work order" (building + room + WO) which quick-adds it and starts the batch. Dispenses are plain transactions on the work order, not added to the stage. Technicians keep the free-text gate. See `docs/mass-staging/phase-10-saved-workorders.md`|
...
**Saved work orders — cards-first gate (role-scoped).** The work-order gate opens on **cards of saved work orders** (`#wo-gate-cards`, from `GET /mass-stages/active-rooms`), shown to **all roles** but **scoped server-side**: a technician sees only work orders assigned to them, a supervisor only ones they created, admin/owner all. Tapping a card starts a scan-and-go batch on that work order (a plain transaction, **not** added to the stage). For **Supervisor+** an **"Add a new work order"** form (`#wo-gate-new`) captures **Community + Unit Number + work order** and an optional **technician assignee**, `POST`s `/mass-stages/quick-room`, then starts the batch; their cards show an `Assigned: <name>` / `Unassigned` badge, and the free-text `#wo-gate-input` remains as a one-off fallback. **Technicians** see only their assigned cards — no quick-add and no free-text box (an empty list reads "No work orders assigned to you"). On the **Mass Stage page**, a community becomes a full card only with **≥ 2 units** (else a lightweight "Single work orders" entry), and each unit's editor has an **"Assign to"** technician dropdown. See `docs/mass-staging/phase-10-saved-workorders.md` and `phase-11-workorder-assignment.md`.
...
**Create + list.** A "New Mass Stage" form (Community, e.g. *Scholars*) plus a list of stages as **expandable one-line cards** (`<details>`): community name + a status pill (`planning` / `loading` / `completed`) + unit/item counts. A card lazy-loads its full detail on first expand.

**Planning (status `planning`).** Inside a stage card, units are nested expandable cards (Unit # — work order, plus an optional **"Assign to"** technician dropdown). Each unit lists planned **item cards** (name, barcode, planned quantity, inline qty edit, remove); an **Add item** search (filters the cached item list) adds estimates — planning is **not** a transaction. **Next Unit** adds a unit (Unit # + work order #). **Save Mass Stage** flips the status to `loading` and locks the plan.
...
|All `/mass-stages` routes require Supervisor or above **except** `GET /active-rooms` (any authenticated; results role-scoped)|Backend (`require_min_role(supervisor)` → 403; active-rooms uses `get_current_user`) + Frontend (nav hidden below Supervisor)|
|Work orders are visible only per role (technician → assigned, supervisor → created, admin/owner → all)|Backend (`list_active_rooms` / `list_stages` scope by `assigned_to_id` / `created_by_id` via `_room_visible_to`)|
|A work order may be assigned only to a technician|Backend (`_validate_assignee` → `InvalidAssigneeError` 400)|
...
|Quick-add a saved work order (scan gate)|POST `/mass-stages/quick-room`|
|List saved work-order cards (scan gate, role-scoped)|GET `/mass-stages/active-rooms`|
|Assign a work order to a technician (or clear)|PATCH `/mass-stages/{id}/rooms/{room_id}/assign`|
...
| workorder-assignment | **Work-order ownership + role-scoped visibility** (Phase 11, `docs/mass-staging/phase-11-workorder-assignment.md`). Each work order (room) records `created_by_id` and an optional `assigned_to_id` (a technician). Visibility: a **technician** sees only work orders assigned to them, a **supervisor** only ones they created, **admin/owner** all — enforced server-side in `list_active_rooms` / `list_stages` and the `GET /mass-stages/active-rooms` gate (lowered to any-authenticated). Assigning is Supervisor+ (`PATCH …/rooms/{id}/assign`, or the quick-add / room-editor dropdowns) and only to technicians (`InvalidAssigneeError`). Technicians scan only their assigned cards (no free-text gate). Plus a **UI-only relabel**: "Building/Room Number" → "Community/Unit Number" (columns unchanged). | Crews need the right people on the right jobs: a tech should see only their assignments, a supervisor only their own work, admins everything. Reusing the room↔work-order model keeps one coherent system; assignment is optional so it never blocks the quick morning flow. |
| saved-workorders | **Saved work orders are mass-staging data — no second system** (Phase 10, `docs/mass-staging/phase-10-saved-workorders.md`). The Scan/Stock gate is **cards-first** for Supervisor+: tappable cards are the active stages' rooms (`GET /mass-stages/active-rooms`), and "Add a new work order" quick-adds **building + room + work order** via `POST /mass-stages/quick-room` (find-or-create the building's active stage, then `add_room`) and starts the batch. Tapping a card runs the existing scan-and-go dispense on that work order — nothing is written back to the stage. **Display threshold:** the Mass Stage page shows a full card only for buildings with **≥ 2 rooms**; single-room buildings are lightweight "Single work orders" entries (data is always a stage). Technicians keep the free-text gate until a future "assign work orders to technicians" update. | The supervisor's morning workflow is to enter work orders once and then tap one when scanning, instead of retyping it every transaction. Reusing the room↔work-order model (rather than a parallel table) keeps the app maximally coherent: a quick-added work order and a planned one are the same object at different detail levels. |
