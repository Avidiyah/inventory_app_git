...
**Saved work orders — cards-first gate (Supervisor+).** For Supervisor+ the work-order gate opens on **cards of the saved work orders** (`#wo-gate-cards`, sourced from `GET /mass-stages/active-rooms` — every room with a work order across active stages); tapping a card starts a scan-and-go batch on that work order. An **"Add a new work order"** form (`#wo-gate-new`) captures **building + room + work order** and `POST`s `/mass-stages/quick-room` (find-or-create the building's active stage, append the room), then starts the batch — the work order persists as a card. Dispenses are plain transactions on the work order and are **not** added to the stage. The free-text `#wo-gate-input` remains as the Technician gate and a Supervisor+ one-off fallback. On the **Mass Stage page**, a building becomes a full card only with **≥ 2 rooms**; single-room buildings show as lightweight "Single work orders" entries. See `docs/mass-staging/phase-10-saved-workorders.md`.
...
|List saved work-order cards (scan gate)|GET `/mass-stages/active-rooms`|
