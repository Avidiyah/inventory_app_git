...
| Priority extraction | `backend/app/integrations/netfacilities/parser.py`, selecting `#priority-level` or `general.get("Priority Level")` | Resolved: the selector was always correct. NetFacilities served a trimmed document omitting the whole Priority row to a session that had not loaded the home page, and a missing row is indistinguishable from an unset priority. The client now primes with one read-only `GET /myhome`, and `has_priority_label()` detects the trimmed view so a stale session is re-primed and re-read instead of writing a blank. |
